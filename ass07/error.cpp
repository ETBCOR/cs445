...
{
    VarDecl * v = (VarDecl *)node_;
    if (!v->usageFlg && !v->usageWrnFlg && !v->isDummy) {
        if (v->isParm)
            throwWrn(PARM_UNUSED, v);
        else if (v->isFun) {
            if (strcmp(v->name, (char *)"main"))
                throwWrn(FUN_UNUSED, v);
        }
        else throwWrn(VAR_UNUSED, v);
    }
}
