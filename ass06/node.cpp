...
void Node::print(bool ann, bool mem)
{
    for (int i = 0; i < MAXCHILDREN; i++)
        if (child[i]) child[i]->print(ann, mem);
    if (sibling) sibling->print(ann, mem);
}
...
if (!semDone) {
    ...
    if (isStatic && natGlob) memType = (char *)"Static";
    else if (isStatic) memType = (char *)"LocalStatic";
    else if (natGlob) memType = (char *)"Global";
    else memType = (char *)"Local";
...
if (!semDone) {
    ...
    Node::semC(tbl);
    if (!isUnscoped) tbl->applyToAll(wrnUnused);
    memSize = fofs;
    if (!isUnscoped) size = tbl->leave();
    Node::semS(tbl);
    semDone = true;
}
}
...
Const::Const(TokenData * tkn)
{
    ...
    switch (token->tokenclass) {
        ...
        case STRINGCONST: type = (char *)"char"; isArray = true;
                          memType = (char *)"Global";
                          memSize = strlen(tkn->svalue) + 1;
    }
}
...
void Const::print(bool ann, bool mem)
{
    ...
    switch (token->tokenclass) {
        ...
        case STRINGCONST: printf("\"%s\" of array", token->svalue); break; 
    }
    printf("%s%s %s[line: %d]\n",
           ann ? " of type " : "",
           ann ? type : "",
